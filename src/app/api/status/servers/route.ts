  ...
  try {
    ...
    const status_list = await Promise.all(
      servers.map(async (server) => {
        const pingStatus = await getPingStatus(server.ip);
        let server_details = { cpu: 'N/A', ram: 'N/A', disk: 'N/A' };

        if (pingStatus.status === 'online') {
          try {
            const response = await axios.get(`http://${server.ip}:8001/status`, { timeout: 2000 });
            if (response.status === 200) {
              ...
              server_details.cpu = parseFloat(cpu_usage_str) || 0.0;
              server_details.ram = parseFloat(ram_usage_str) || 0.0;

              const used_bytes = agent_data.disk_used || 0;
              const total_bytes = agent_data.disk_total || 0;
              server_details.disk_percent = total_bytes > 0 ? (used_bytes / total_bytes) * 100 : 0.0;
            }
          } catch (error) {
            // Agent not reachable
          }
        }
        return { ...server, ...pingStatus, details: server_details, type: 'server' };
      })
    );
